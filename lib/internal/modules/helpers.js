...
/**
 * Get the conditions array a user hook should observe given the request type. For
 * `'require'` requests this is the CJS conditions array (`['require', 'node', ...]`),
 * matching what the CJS loader would pass. For `'import'` requests it is the caller's
 * default ESM conditions (typically `['node', 'import', ...]`). The ESM defaults are
 * passed in because they are per-loader-instance (see `ModuleLoader.#defaultConditions`).
 * @param {'import'|'require'} requestType
 * @param {string[]} defaultEsmConditions
 * @returns {string[]}
 */
function getConditionsForRequestType(requestType, defaultEsmConditions) {
  return requestType === 'require' ? getCjsConditionsArray() : defaultEsmConditions;
}
