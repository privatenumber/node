...
/**
 * Convert the internal ModuleRequestType symbol into the user-facing string used as
 * `context.requestType` on resolve and load hooks. `kRequireInImportedCJS` is the only one
 * with `require()` semantics; `kImportInRequiredESM` is an `import` statement inside a
 * `require()`'d ESM, which is still semantically an `import`. This matches the existing
 * `WATCH_REPORT_DEPENDENCIES` telemetry mapping in `esm/loader.js`.
 * @param {ModuleRequestType} requestType
 * @returns {'import'|'require'}
 */
function toRequestTypeString(requestType) {
  return requestType === kRequireInImportedCJS ? 'require' : 'import';
}
