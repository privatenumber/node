...
class AsyncLoaderHooksOnLoaderHookWorker {
  ...
  async resolve(
    ...
    const context = {
      conditions: requestType === 'require' ? getCjsConditionsArray() : getDefaultConditions(),
      importAttributes,
      parentURL,
      requestType,
    };
  ...
  async load(url, context = {}) {
    // The context arriving here was constructed on the main thread (see ModuleLoader
    // #getOrCreateModuleJobAfterResolve) and carries `requestType`. Fill in `conditions` so
    // user load hooks have parity with sync hooks (which receive conditions via
    // ModuleLoadContext). Default requestType to 'import' if the caller didn't set it.
