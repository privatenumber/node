...
class ModuleLoader {
  ...
  #getOrCreateModuleJobAfterResolve(parentURL, resolveResult, request, requestType) {
    ...
    // Tell user hooks whether this load is on behalf of a require() call (`'require'`)
    // or an import statement / dynamic import() / import.meta.resolve() (`'import'`).
    // Only kRequireInImportedCJS represents `require()` semantics here; kImportInRequiredESM
    // is the import statement inside a require()'d ESM, which is still an import.
    const requestTypeString = requestType === kRequireInImportedCJS ? 'require' : 'import';
    const context = { format, importAttributes, requestType: requestTypeString, __proto__: null };
  ...
  getOrCreateModuleJob(parentURL, request, requestType) {
    // Tell user hooks whether this resolve is on behalf of a require() call or an import.
    // Only kRequireInImportedCJS means "for require()"; kImportInRequiredESM is an
    // import statement inside a require()'d ESM and still has import semantics.
    const requestTypeString = requestType === kRequireInImportedCJS ? 'require' : 'import';

    let maybePromise;
    if (requestType === kRequireInImportedCJS || requestType === kImportInRequiredESM) {
      // In these two cases, resolution must be synchronous.
      maybePromise = this.resolveSync(parentURL, request, false, requestTypeString);
      assert(!isPromise(maybePromise));
    } else {
      maybePromise = this.#resolve(parentURL, request, requestTypeString);
    }
   ...
   *   It's undefined if it's from the root module.
   * @param {ModuleRequest} request Module request.
   * @param {'import'|'require'} [requestType] Whether this resolve is on behalf of a require() call
   *   or an import. Defaults to 'import'.
   * @returns {Promise<ResolveResult>|ResolveResult}
   */
  #resolve(parentURL, request, requestType = 'import') {
    if (this.isForAsyncLoaderHookWorker) {
      ...
      return this.#asyncLoaderHooks.resolve(specifier, parentURL, importAttributes, requestType);
    }

    return this.resolveSync(parentURL, request, false, requestType);
  }
   ...
   * @param {{ parentURL?: string, importAttributes: ImportAttributes, conditions?: string[],
   *           requestType?: 'import'|'require' }} context See {@link resolveSync}.
   * @returns {{ format: string, url: string }}
   */
   ...
   * @param {'import'|'require'} [requestType] Whether this resolve is on behalf of a `require()` call
   *   or an `import`. Defaults to `'import'`. When `'require'`, resolution uses CJS conditions instead
   *   of ESM defaults, matching what the CJS loader would do.
   * @returns {ResolveResult}
   */
  resolveSync(parentURL, request, shouldSkipSyncHooks = false, requestType = 'import') {
    const specifier = `${request.specifier}`;
    const importAttributes = request.attributes ?? kEmptyObject;
    ...
    const conditions = requestType === 'require' ? getCjsConditionsArray() : this.#defaultConditions;
    ...
    if (!shouldSkipSyncHooks && syncResolveHooks.length) {
      ...
      result = resolveWithSyncHooks(specifier, parentURL, importAttributes, conditions, requestType,
  ...
  #loadSync(url, context) {
    ...
    // Default requestType to 'import' for callers (entry point loading, etc.) that don't set it.
    const requestType = context.requestType ?? 'import';
    // For `require(esm)` bridge calls the load is semantically a require(), so user hooks should
    // see CJS conditions, matching what the CJS loader would pass. For regular imports and
    // imports in required ESM, the conditions remain ESM defaults.
    const conditions = requestType === 'require' ? getCjsConditionsArray() : this.#defaultConditions;
    let result;
