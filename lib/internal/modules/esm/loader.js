...
class ModuleLoader {
  ...
  #getOrCreateModuleJobAfterResolve(parentURL, resolveResult, request, requestType) {
    ...
    if (process.env.WATCH_REPORT_DEPENDENCIES && process.send) {
      process.send({ [`watch:${toRequestTypeString(requestType)}`]: [url] });
    }
    ...
    // Tell user hooks whether this load is on behalf of a require() call or an import.
    // See {@link toRequestTypeString} for the mapping semantics.
    const context = {
      format,
      importAttributes,
      requestType: toRequestTypeString(requestType),
      __proto__: null,
    };
  ...
  getOrCreateModuleJob(parentURL, request, requestType) {
    ...
    // See {@link toRequestTypeString} for the mapping semantics.
    const requestTypeString = toRequestTypeString(requestType);
  ...
  resolveSync(parentURL, request, shouldSkipSyncHooks = false, requestType = 'import') {
    ...
    const conditions = getConditionsForRequestType(requestType, this.#defaultConditions);
